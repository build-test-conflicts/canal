...
public class MessageUtil {
      ...
      if (!rowChange.getIsDdl()) {
        ...
        for (        CanalEntry.RowData rowData : rowChange.getRowDatasList()) {
          ...
          for (          CanalEntry.Column column : columns) {
            ...
            row.put(column.getName(),JdbcTypeUtil.typeConvert(dml.getTable(),column.getName(),column.getValue(),column.getSqlType(),column.getMysqlType()));
          ...
          if (eventType == CanalEntry.EventType.UPDATE) {
            ...
            for (            CanalEntry.Column column : rowData.getBeforeColumnsList()) {
              if (updateSet.contains(column.getName())) {
                rowOld.put(column.getName(),JdbcTypeUtil.typeConvert(dml.getTable(),column.getName(),column.getValue(),column.getSqlType(),column.getMysqlType()));
              }
            }
            if (!rowOld.isEmpty()) {
              old.add(rowOld);
  ...
  public static List<Map<String,Object>> changeRows(  String table,  List<Map<String,String>> rows,  Map<String,Integer> sqlTypes,  Map<String,String> mysqlTypes){
